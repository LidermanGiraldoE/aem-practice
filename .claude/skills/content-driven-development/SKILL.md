...
## Step 3b: Define Data Model, Row Mapping & Bridge Pattern

**Skip if:** CSS-only changes that don't affect content structure or model fields

**Invoke:** block-data-modeling skill

**Provide:**
- Content model output from Step 3 (field list, block type)
- Block name and rendering approach (`native` or `preact`)

**The block-data-modeling skill will:**
- Classify block type (simple / container-items / config-items)
- Generate `blocks/<name>/_<name>.json` with definitions, models, filters
- Produce a row-index mapping (field → DOM row → JS read pattern)
- Select and template the bridge pattern (`decorate()` starter code)
- Validate the model against 8 rules (V-001 through V-008)

**Success criteria:**
- ✅ `_<name>.json` written and passes `npm run build:json`
- ✅ Row-index mapping documented
- ✅ Bridge pattern template produced
- ✅ All validation rules PASS

**Mark todo complete when:** Model JSON built, row mapping documented, bridge pattern ready
...
- **block-data-modeling**: Invoked in Step 3b to generate the `_<name>.json` model, row-index mapping, and bridge pattern
